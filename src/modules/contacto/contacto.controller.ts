...
import { routeContactoAgregar, routeContactoLista } from 'src/shared/utils/api/api.route';
...
export class ContactoController {
  ...
  async create(@Param('user') user: string, @Body() array: [ContactoDto]): Promise<ApiResult> {
    let apiResult = { title: routeContactoAgregar.title, route: routeContactoAgregar.route, status: 'error', code: 0, message: '', boolean: false, rows: 0, data: null } as ApiResult;
